...
Our project is focused on investigating the possibility of exploiting these decoders to learn about changes in gene expression patterns as a cell transitions from healthy to sick.
...
We labeled the health score of each artifical cell as its distance from the sick end of the line (the healthy cell itself has a healthy score of 1, the sick cell has a healthy score of 0, an artifical cell located halfway on the line connecting the healthy and sick cells has a healthy score of .5).
We then use Hotspot \citep{detomaso_hotspot_2021} to 
